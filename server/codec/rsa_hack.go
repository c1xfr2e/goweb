...
func PriKeyDECRYPT(priv *rsa.PrivateKey, input []byte) ([]byte, error) {
	output := bytes.NewBuffer(nil)
	err := priKeyIO(priv, bytes.NewReader(input), output, false)
	if err != nil {
		return []byte(""), err
	}
...
// EncryptWithPrivateKey encrypt msg with RSA private key
// receive a bytes msg and return the encrypted bytes msg
func EncryptWithPrivateKey(priv *rsa.PrivateKey, msg []byte) ([]byte, error) {
	output := bytes.NewBuffer(nil)
	err := priKeyIO(priv, bytes.NewReader(msg), output, true)
	if err != nil {
		return []byte(""), err
	}
	return ioutil.ReadAll(output)
...
func DecryptWithPublicKey(pub *rsa.PublicKey, msg []byte) ([]byte, error) {
	output := bytes.NewBuffer(nil)
	err := pubKeyIO(pub, bytes.NewReader(msg), output)
	if err != nil {
		return []byte(""), err
	}
	return ioutil.ReadAll(output)
...
// Code bellow are copied from crypto/rsa.
